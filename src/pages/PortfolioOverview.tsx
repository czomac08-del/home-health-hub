...
const PortfolioOverview = () => {
  ...
  // Only count properties with a real, verified score. Never fabricate a default of 50.
  const scoredProperties = profileProperties.filter((p: any) => typeof p.health_score === "number");
  const avgHealth = scoredProperties.length > 0
    ? Math.round(scoredProperties.reduce((sum, p: any) => sum + p.health_score, 0) / scoredProperties.length)
    : null;

  const needsAttention = scoredProperties.filter((p: any) => p.health_score < 70);
    ...
    <div className="min-h-screen pb-24 max-w-lg lg:max-w-6xl mx-auto px-6 py-8">
      ...
      <div className="flex items-center gap-6 rounded-xl border border-border bg-card p-5 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {avgHealth === null ? "No verified scores yet" : "Average across scored properties"}
          </p>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <p className="text-2xl font-bold text-foreground">{avgHealth === null ? "—" : `${avgHealth}%`}</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        {profileProperties.length === 0 ? (
          ...
          profileProperties.map((prop, i) => (
            ...
            >
              <HealthRing percentage={typeof prop.health_score === "number" ? prop.health_score : null} size={40} />
